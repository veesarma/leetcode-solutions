...
def main():
    parser = argparse.ArgumentParser(
        prog="leetcode", description="Run LeetCode problems by name with JSON input."
    )
    sub = parser.add_subparsers(dest="cmd", required=True)

    sub.add_parser("list", help="List available problems.")
    p_run = sub.add_parser("run", help="Run a specific problem.")
    p_run.add_argument("problem", help="Problem slug (e.g., tßwo-sum or two_sum).")
    ...
    if data is None:
        raise SystemExit(
            "No input provided. Use --input JSON, --input-file FILE, or pipe JSON via stdin."
        )
    ...
    try:
        ...
        raise SystemExit(
            f"Problem '{args.problem}' not found. Available: {', '.join(_available_problems())}"
        ) from e
